<div x-data ="{
    ...
    <div
        ...
        <div class="w-full ">
            ...
            <div class="relative">
                ...
                <flux:input id="buscar-lic" type="text" wire:model.live="search"
                    placeholder="Buscar por nombre, nombre corto o RVOE…" class="pl-10 w-full" />
            </div>
        </div>
    ...
    <div class="relative">
        ...
        <div class="transition ease-out duration-200" wire:loading.class="blur-sm opacity-80 pointer-events-none"
            wire:target="search">
            <div
                class="overflow-hidden  rounded-2xl border border-gray-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 shadow">
                                    ...
                                    <td colspan="6" class="px-6 py-10 text-center text-gray-500 dark:text-gray-400">
                                        <div class="mx-auto w-full max-w-md">
                                            <div
                                                ...
                                                <p class="text-sm">Intenta ajustar tu búsqueda o crea una nueva
                                                    licenciatura.</p>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <tr class="hover:bg-gray-50/60 dark:hover:bg-neutral-800/40">
                                        ...
                                        <td class="px-4 py-3">
                                            <div class="font-medium text-gray-900 dark:text-white">
                                                {{ $licenciatura->nombre }}</div>
                                            @if ($licenciatura->slug)
                                                <div class="text-xs text-gray-500 dark:text-gray-400">
                                                    /{{ $licenciatura->slug }}</div>
                                            @endif
                                        </td>
                                        <td class="px-4 py-3 text-gray-800 dark:text-gray-200">
                                            {{ $licenciatura->nombre_corto }}</td>
                                        <td class="px-4 py-3">
                                            @if ($licenciatura->RVOE)
                                                <span
                                                    class="inline-flex items-center rounded-full border border-emerald-300/60 bg-emerald-50 px-2.5 py-0.5 text-xs font-medium text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-300 dark:border-emerald-700/50">
                                                    RVOE: {{ $licenciatura->RVOE }}
                                                </span>
                                            @else
                                                <span
                                                    class="inline-flex items-center rounded-full border border-gray-300/70 bg-gray-50 px-2.5 py-0.5 text-xs font-medium text-gray-600 dark:bg-neutral-800/60 dark:text-gray-300 dark:border-neutral-700">
                                                    Sin RVOE
                                                </span>
                                            @endif
                                        </td>
