      ...
      <form wire:submit.prevent="actualizarLicenciatura" class="space-y-6">
        <flux:field>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div class="space-y-4">
              <flux:input
                wire:model.live="logo_nueva"
                badge="Opcional"
                :label="__('Logo de la licenciatura')"
                type="file"
                accept="image/jpeg,image/jpg,image/png"
              />

              <div wire:loading wire:target="logo_nueva" class="flex items-center gap-2 rounded-lg border border-zinc-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 px-3 py-2">
                <svg class="h-5 w-5 animate-spin text-blue-600 dark:text-blue-400" viewBox="0 0 24 24" fill="none">
                  <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                  <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"></path>
                </svg>
                <span class="text-sm text-zinc-700 dark:text-zinc-300">Cargando logo…</span>
              </div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="rounded-xl border border-zinc-200 dark:border-neutral-800 bg-zinc-50 dark:bg-neutral-800/40 p-4 text-center">
                  <p class="text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-3">Logo actual</p>
                  @if ($logo)
                    <img src="{{ asset('storage/licenciaturas/' . $logo) }}" alt="Logo de la licenciatura" class="mx-auto h-24 w-24 rounded-xl object-cover shadow">
                  @else
                    <div class="mx-auto h-24 w-24 rounded-xl bg-zinc-200 dark:bg-neutral-700 grid place-items-center text-zinc-500 dark:text-zinc-300 text-xs">Sin Logo</div>
                  @endif
                </div>

                <div class="rounded-xl border border-zinc-200 dark:border-neutral-800 bg-zinc-50 dark:bg-neutral-800/40 p-4 text-center">
                  <p class="text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-3">Nuevo Logo</p>
                  @if ($logo_nueva)
                    <img src="{{ $logo_nueva->temporaryUrl() }}" alt="Nuevo logo de la licenciatura" class="mx-auto h-24 w-24 rounded-xl object-cover shadow">
                  @else
