<div>
    ...
    <div x-data="{ open: false }" class="my-4">
        <!-- Toggle -->
        <button type="button" @click="open = !open" :aria-expanded="open" aria-controls="panel-crear-profesor"
        ...
        <!-- Panel -->
        <div id="panel-crear-profesor" x-show="open" x-cloak x-transition:enter="transition ease-out duration-250"
                                ...
                                <div class="flex items-start gap-3">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <p class="text-xs opacity-90">
                                            Los datos de CURP se obtienen en pruebas y <b>no</b> se consulta la API
                                            real.
                                        </p>
                                    </div>
                                </div>
                        ...
                        <flux:field>
                            ...
                            <div class="mt-5 grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div x-data="{
                                    ...
                                    <!-- Zona Drag & Drop -->
                                    <div class="group relative flex flex-col items-center justify-center gap-3 rounded-xl border border-dashed border-gray-300 dark:border-neutral-700 bg-gradient-to-br from-gray-50 to-white dark:from-neutral-900 dark:to-neutral-950 p-6 text-center cursor-pointer hover:border-blue-400/70 hover:from-blue-50 hover:to-white dark:hover:border-blue-500/40 transition"
                                        @click="$refs.fileInput.click()" @dragover.prevent @dragenter.prevent
                                        @drop="onDrop">
                                        <div
                                            class="inline-flex size-12 items-center justify-center rounded-full ring-1 ring-gray-200/70 dark:ring-neutral-700/70 shadow-sm">
                                            <!-- Ícono -->
                                        ...
                                        <div>
                                            <p class="font-medium text-gray-800 dark:text-gray-200">Sube la foto del
                                                profesor</p>
                                            <p class="text-sm text-gray-500 dark:text-gray-400">
                                                Arrastra y suelta la imagen aquí o
                                                <span
                                                    class="text-blue-600 dark:text-blue-400 underline underline-offset-4">haz
                                                    clic para seleccionar</span>
                                            </p>
                                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Recomendado: hasta
                                                1MB (JPG o PNG)</p>
                                        </div>
                                    </div>

                                    <!-- Input oculto (Livewire) -->
                                    <input x-ref="fileInput" type="file" class="hidden" wire:model.live="foto"
                                        accept="image/jpeg,image/jpg,image/png">

                                    <!-- Loader -->
                                    <div wire:loading wire:target="foto"
                                        class="mt-3 flex items-center gap-2 text-blue-600 dark:text-blue-400">
                                    ...
                                    @if ($foto)
                                        ...
                                        <div
                                            class="mt-5 flex items-center justify-between gap-4 rounded-xl border border-gray-200 dark:border-neutral-800 p-3">
                                            <div class="flex items-center gap-3">
                                                <button type="button" class="relative block" @click="open=true"
                                                ...
                                                <div class="text-sm">
                                                    <p class="font-medium text-gray-800 dark:text-gray-200">Archivo
                                                        listo</p>
                                                    <p class="text-xs text-gray-500 dark:text-gray-400">Haz clic en la
                                                        miniatura para ampliarla</p>
                                                </div>
                                            </div>
                                ...
                                <div class="space-y-4">
                                    <flux:select badge="Requerido" label="Usuario" wire:model.live="user_id">
                                        ...
                                        @foreach ($usuarios as $key => $usuario)
                                            <option value="{{ $usuario->id }}">{{ $key + 1 }}.-
                                                {{ $usuario->username }} => {{ $usuario->email }}</option>
                                        @endforeach
                                    </flux:select>

                                    <flux:input type="text" badge="Requerido" label="Nombre" placeholder="Nombre"
                                        wire:model.live="nombre" />
                                    <flux:input type="text" badge="Opcional" label="Apellido Materno"
                                        placeholder="Apellido Materno" wire:model="apellido_materno" />

                                    <flux:input type="text" badge="Opcional" label="Perfil" placeholder="Perfil"
                                        wire:model="perfil" />
                                </div>

                                <!-- Columna 3: con espacio vertical -->
                                <div class="space-y-4">
                                    <flux:input type="text" label="CURP" placeholder="CURP" badge="Requerido"
                                        wire:model.live="CURP" value="{{ $CURP }}" />

                                    <div wire:loading wire:target="CURP"
                                        class="md:col-span-2 flex items-center gap-2 text-blue-600">
                                    ...
                                    <flux:input type="text" badge="Requerido" label="Apellido Paterno"
                                        placeholder="Apellido Paterno" wire:model="apellido_paterno" />
                                    <flux:input type="text" badge="Opcional" label="Teléfono"
                                        placeholder="Teléfono" wire:model="telefono" />
                                    <flux:input type="color" badge="Opcional" label="Color" placeholder="Color"
                                        wire:model="color" />
                                </div>
