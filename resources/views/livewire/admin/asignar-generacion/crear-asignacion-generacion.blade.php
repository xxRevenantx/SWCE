<div>
    ...
    <div class="flex flex-col gap-4">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Asignar Generación</h1>
        <p class="text-sm text-gray-600 dark:text-gray-400">Formulario para asignar una generación a una licenciatura</p>
    </div>

    <!-- form-pro -->
    <div x-data="{ open: false }" class="my-4">
        <!-- Toggle -->
        <button type="button" @click="open = !open" :aria-expanded="open" aria-controls="panel-asignar-generacion"
            ...
            <span class="font-medium">{{ __('Asignar generación') }}</span>
        ...
        <!-- Panel -->
        <div id="panel-asignar-generacion" x-show="open" x-cloak x-transition:enter="transition ease-out duration-250"
            ...
            <form wire:submit.prevent="asignarGeneracion" class="group">
                <!-- Card -->
                <div
                    ...
                    <div class="p-5 sm:p-6 lg:p-8">
                        ...
                        <div class="mb-5 flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Asignar generación
                                    </h2>
                                    <p class="text-xs text-gray-500 dark:text-gray-400">Selecciona la licenciatura y la
                                        generación.</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <flux:field>
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                <flux:select badge="Requerido" label="Selecciona una licenciatura"
                                    wire:model="licenciatura_id">
                                    <flux:select.option value="0">--Selecciona una licenciatura--
                                    </flux:select.option>
                                ...
                                <flux:select badge="Requerido" label="Selecciona una generación"
                                    wire:model="generacion_id">
                                    <flux:select.option value="0">--Selecciona una generación--
                                    </flux:select.option>
                                    @foreach ($generaciones as $generacion)
                                        <flux:select.option value="{{ $generacion->id }}">{{ $generacion->generacion }}
                                        </flux:select.option>
                                    @endforeach
                                </flux:select>
                            </div>
